...
router.post("/profile/analyze", async (req: Request, res: Response) => {
  try {
    ...
    const opportunities = await db.opportunity.find();
    for (const opp of opportunities) {
      try {
        const requirementsList: string[] = Array.isArray(opp.requirements)
          ? opp.requirements
          : JSON.parse(opp.requirements || "[]");
          
        const matchData = await compareOpportunityWithAI(parsed.skills, parsed.summary, {
          title: opp.title,
          company: opp.company,
        ...
        await db.matchResult.findOneAndUpdate(
          { userId: profile.id, opportunityId: opp.id },
          {
            userId: profile.id,
...
router.get("/opportunities", async (req: Request, res: Response) => {
  try {
    ...
    const opportunities = await db.opportunity.find().sort({ createdAt: -1 });

    // Fetch matches for this specific user
    const matches = await db.matchResult.find({ userId });

    // Fetch user application statuses
    const applications = await db.applicationProgress.find({ userId });

    // Convert mongoose documents to JSON
    const opportunityDocs = opportunities.map((opp: any) => opp.toJSON());
    const matchDocs = matches.map((m: any) => m.toJSON());
    const appDocs = applications.map((a: any) => a.toJSON());
...
function getMockRepos(username: string) {
  return [
    {
