...
import { generateDietChart } from "./agents/nutritionCoach";
...
router.post("/okx/mcp", async (req: Request, res: Response) => {
  try {
    ...
    if (method === "call_tool" || req.body.type === "call_tool") {
      ...
      switch (toolName) {
        case "generate_diet_chart": {
          const result = await generateDietChart(
            args.goals,
            args.dietaryPreferences,
            args.allergies,
            args.currentRecoveryLevel
          );
          return res.json({
        ...
        case "analyze_sleep_and_plan_bedtime": {
          const result = await analyzeSleepAndPlanBedtime(
            Number(args.sleepHours),
            Number(args.sleepQuality),
            !!args.caffeineAfternoon,
            !!args.screenTimeBeforeBed
          );
          return res.json({
            content: [{ type: "text", text: JSON.stringify(result, null, 2) }]
          });
        ...
        case "optimize_wellness_collaboration": {
          const result = await optimizeWellnessCollaboration(
            Number(args.sleepHours),
            Number(args.sleepQuality),
            Number(args.stressLevel),
            args.currentFitnessPlan || "Standard workout"
          );
          return res.json({
            content: [{ type: "text", text: JSON.stringify(result, null, 2) }]
          });
...
router.post("/agent/collaborate", async (req: Request, res: Response) => {
  try {
    const {
      ...
      fitnessGoals = "General Fitness & Health",
      limitations = "None",
    ...
    const nexusResult = await optimizeWellnessCollaboration(
      Number(sleepHours),
      Number(sleepQuality),
      Number(stressLevel),
      currentFitnessPlan || "Standard workout"
    );
    ...
    const adaptedWorkout = await generateWorkoutPlan(
      fitnessLevel,
      fitnessGoals,
      limitations,
      recoveryReason
    ...
    const adaptedDiet = await generateDietChart(
      fitnessGoals,
      dietaryPreferences,
      allergies,
      nutritionRecovery
    ...
    const bedtimeRoutine = await analyzeSleepAndPlanBedtime(
      Number(sleepHours),
      Number(sleepQuality),
      caffeineAfternoon,
      screenTimeBeforeBed
